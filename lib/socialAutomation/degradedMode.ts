...
export function buildDegradedState(connections: PlatformConnection[], queue: PublishQueueItem[]): DegradedState {
  const platformNotConnected = connections.filter((item) => item.state === "not_connected" || item.state === "auth_url_ready").map((item) => item.platform);
  ...
  const auditRestricted = connections.filter((item) => item.state === "restricted" || item.auditRestricted).map((item) => item.platform);
