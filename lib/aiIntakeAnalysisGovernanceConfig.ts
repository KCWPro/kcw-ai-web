...
/**
 * Phase 2 internal governance normalization layer.
 * Resolves policy + rollout/rollback + env overrides into an effective config.
 */
...
const DEFAULT_ENABLED_POLICIES: IntakeAnalysisPolicyName[] = ["standard"];
...
function normalizePolicy(raw: string | undefined, fallback: IntakeAnalysisPolicyName, policyAdjustments: string[], reasonLabel: string): IntakeAnalysisPolicyName {
  const value = (raw || "").trim().toLowerCase();
  if (!value) return fallback;

  if (value === "standard" || value === "conservative") return value;

  policyAdjustments.push(`${reasonLabel} fallback ${value} -> ${fallback}`);
  return fallback;
}

function parseEnabledPolicies(raw: string | undefined, warnings: string[]): IntakeAnalysisPolicyName[] {
  if (!raw || !raw.trim()) {
    return [...DEFAULT_ENABLED_POLICIES];
  }
  ...
  const enabled = new Set<IntakeAnalysisPolicyName>();
  const invalid: string[] = [];

  for (const value of values) {
    if (value === "standard" || value === "conservative") {
      enabled.add(value);
    } else {
      invalid.push(value);
    }
  ...
  if (invalid.length > 0) {
    warnings.push(`INTAKE_ANALYSIS_ENABLED_POLICIES invalid entries ignored: ${invalid.join(",")}`);
  }

  if (enabled.size === 0) {
    warnings.push("INTAKE_ANALYSIS_ENABLED_POLICIES empty after normalization; fallback to standard");
    return [...DEFAULT_ENABLED_POLICIES];
  }
...
export function resolveIntakeAnalysisGovernanceConfig(
  ...
  const resolvedPolicy = normalizePolicy(configuredPolicyName || process.env.INTAKE_ANALYSIS_POLICY, DEFAULT_POLICY, policyAdjustments, "INTAKE_ANALYSIS_POLICY");

  const enabledPolicies = parseEnabledPolicies(process.env.INTAKE_ANALYSIS_ENABLED_POLICIES, warnings);

  let finalPolicy: IntakeAnalysisPolicyName = resolvedPolicy;
  let rolloutBlocked = false;
  let rolloutReason: "none" | "policy_not_enabled" = "none";

  if (!enabledPolicies.includes(resolvedPolicy)) {
    finalPolicy = DEFAULT_POLICY;
    rolloutBlocked = true;
    rolloutReason = "policy_not_enabled";
    policyAdjustments.push(`policy rollout guard: ${resolvedPolicy} -> ${DEFAULT_POLICY}`);
  }

  const requestedRollbackPolicy = process.env.INTAKE_ANALYSIS_FORCE_POLICY;
  const resolvedRollbackPolicy = normalizePolicy(requestedRollbackPolicy, DEFAULT_POLICY, policyAdjustments, "INTAKE_ANALYSIS_FORCE_POLICY");
  const rollbackSwitchApplied = !!(requestedRollbackPolicy && requestedRollbackPolicy.trim());

  if (rollbackSwitchApplied) {
    finalPolicy = resolvedRollbackPolicy;
    policyAdjustments.push(`rollback switch applied -> ${resolvedRollbackPolicy}`);
  }

  const policy = getIntakeAnalysisGovernancePolicy(finalPolicy);
  ...
  return {
    ...
    enabled_policies_snapshot: enabledPolicies,
    rollback_switch_applied: rollbackSwitchApplied,
    rollback_target_policy: rollbackSwitchApplied ? finalPolicy : null,
