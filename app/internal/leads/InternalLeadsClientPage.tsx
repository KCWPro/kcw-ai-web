...
export default function InternalLeadsClientPage({ initialLeads }: { initialLeads: InternalLeadRow[] }) {
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState<"all" | LeadStatus>("all");
  const [leads, setLeads] = useState<InternalLeadRow[]>(initialLeads);
  ...
  useEffect(() => {
    const syncSearchParams = () => setSearchQueryString(window.location.search);
    syncSearchParams();
  ...
  useEffect(() => {
    ...
    setFollowUpScope(rawFollowUpScope === "available" ? "available" : null);
    setEstimateScope(rawEstimateScope === "available" ? "available" : null);
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter((lead) => {
      ...
      if (hasExplicitPinnedIds) return matchDrillDownIds;
    ...
    <main className="px-4 py-8 text-slate-900 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-3 rounded-2xl bg-white p-4 shadow-sm sm:grid-cols-2">
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by customer, phone, city, service..." className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none ring-slate-900 focus:ring" />
          <div className="flex items-center gap-2">
            <select value={status} onChange={(event) => setStatus(event.target.value as "all" | LeadStatus)} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none ring-slate-900 focus:ring">
            ...
            <button type="button" onClick={loadLeads} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700">
              Refresh
            </button>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 shadow-sm">
          ...
          {(followUpScope === "available" || estimateScope === "available" || drillDownMode === "needs_review") && drillDownIds !== null ? (
            <p className="mt-1 text-xs text-slate-600">Dashboard drill-down is pinned to runtime snapshot IDs ({drillDownIds.size}) to keep counts aligned.</p>
          ) : null}
          {queueFilter !== "all" ? (
            <Link href="/internal/leads" className="mt-2 inline-flex items-center font-semibold text-slate-900 underline decoration-slate-300 underline-offset-4 hover:text-slate-700">
        ...
        <section className="overflow-hidden rounded-2xl bg-white shadow-sm">
          ...
          {!loading && filteredLeads.length === 0 ? <p className="px-4 py-8 text-center text-sm text-slate-500">No matching leads found.</p> : null}
          {loading ? <p className="px-4 py-3 text-center text-sm text-slate-500">Refreshing leads…</p> : null}
        </section>
      </div>
    </main>
  );
