...
export async function POST(req: Request) {
  ...
  const updated = inMemoryConnectionPersistence.upsertConnection(body.platform, { tokenExpiresAt: new Date(Date.now() + 60 * 60_000).toISOString(), lastSyncedAt: new Date().toISOString() });
  if (!updated) return Response.json({ success: false, error: "connection_not_found" }, { status: 404 });

  return Response.json({ success: true, connection: deriveConnectionState(updated), note: "Refresh endpoint contract ready; connect official refresh token exchange." });
}
